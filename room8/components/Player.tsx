...
const RemoteAudioController = ({ stream, position }: { stream: MediaStream | undefined, position: [number, number, number] }) => {
    ...
    return <Html><audio ref={audioRef} autoPlay playsInline /></Html>;
};

// --- Reaction Particle Subcomponent using CSS Animation ---
const ReactionParticle = ({ emoji, onComplete }: { emoji: string, onComplete: () => void }) => {
  useEffect(() => {
...
const DynamicMicIcon = ({ stream, isMicOn }: { stream: MediaStream | null, isMicOn: boolean }) => {
    ...
    return (
        <div className="relative w-4 h-4">
            ...
            <div 
                className="absolute bottom-0 left-0 w-full overflow-hidden" // REMOVED transition-all duration-75 for instant response
...
// --- Remote Player ---
export const RemotePlayer = ({ id, data }: { id: string; data: any }) => {
  const group = useRef<THREE.Group>(null);
  const { friends, addFriend, removeFriend, updatePeer, activeRoomId, rooms, peerStreams, language } = useStore();
  ...
  useFrame((state) => {
    if (group.current) {
      ...
      if (dist > 60 && isVisible) setIsVisible(false);
      else if (dist <= 60 && !isVisible) setIsVisible(true);
    }
  });
            ...
            {!data.isBot && (
                ...
                {!isFriend && (
                    <button 
                        ...
                        onClick={(e) => { e.stopPropagation(); addFriend(id); setShowMenu(false); }}
                    >
                        {t('add_friend')}
                    </button>
                )}
...
export const LocalPlayer = () => {
  ...
  useFrame((state) => {
      if(group.current && (state.controls as any)) {
