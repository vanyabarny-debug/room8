...
import { PlayerConfig, PlayerState, RoomConfig, TeleportRequest, FriendRequest, Language } from './types';
...
interface AppState {
  ...
  activeCinemaStream: MediaStream | null; // Changed from cinemaMode boolean to specific stream
  ...
  addFriend: (id: string) => void; // Internal use
...
export const useStore = create<AppState>((set, get) => ({
  ...
  joinRoom: (roomId) => {
    ...
    // Check rules if room exists in local store, otherwise defaults are fine for network
  ...
  removePeer: (id) => set((state) => {
    ...
    // If we were watching this peer's stream, close it
    let newCinemaStream = state.activeCinemaStream;
    // Check implies looking up stream ID match, simplest is just to close if peer leaves
    // But since streams are separate, we can check later.
    // For safety, if the peer leaves, their streams are gone.
  ...
  addStream: (id, stream, meta) => set((state) => {
      ...
      // FORCE STATE UPDATE for indicators (Red Mic -> Green Mic)
      const peerUpdate: Partial<PlayerState> = {};
      if (isScreen) {
          peerUpdate.isScreenSharing = true;
      } else {
          peerUpdate.isMicOn = true;
      }
      ...
      if (newPeers[id]) {
          ...
          // If peer doesn't exist yet (rare race condition), create placeholder
           newPeers[id] = {
              id, name: 'Loading...', color: '#fff', shape: 'box',
              position: [0,0,0], rotation: [0,0,0],
              isMoving: false, isSpeaking: false, 
              isMicOn: !isScreen, 
  ...
  // FRIEND SYSTEM
  requestFriend: (id) => {
      networkService.sendFriendRequest(id, get().localPlayer.name);
  },
