...
interface AppState {
  ...
  // Conference State (Local visual state, realistically would sync via network)
  conferenceState: {
  ...
  isSitting: boolean;
  isSpeaker: boolean; // Local player is speaker?
...
export const useStore = create<AppState>((set, get) => ({
  ...
  conferenceState: {
      banners: {},
      lightColor: '#ff0000', // Start with Rainbow Red
      screenVideoUrl: null,
  ...
  updatePeer: (id, data) => set((state) => {
    ...
    const existing = state.peers[id] || {
      ...
      isMoving: false, isSpeaking: false, isMicOn: false, isScreenSharing: false, isSpeaker: false
    };
  ...
  addStream: (id, stream, meta) => set((state) => {
      ...
      if (newPeers[id]) newPeers[id] = { ...newPeers[id], ...peerUpdate };
          ...
          isMicOn: !isScreen, isScreenSharing: isScreen, isSpeaker: false
      };
