...
class NetworkService {
  ...
  private retryAddStreams(peerId: string, attempt = 1) {
      ...
      const { micStream, screenStream, videoStream } = useStore.getState();
      
      try {
          if (micStream) this.room.addStream(micStream, peerId, { type: 'audio' });
          if (videoStream) this.room.addStream(videoStream, peerId, { type: 'video' });
  ...
  broadcastMyState(force = false) {
    ...
    const pos = state.gameRefs.localPlayerPosition;
    const rot = state.gameRefs.localPlayerRotation;

    const payload = {
        name: state.localPlayer.name,
        color: state.localPlayer.color,
        shape: state.localPlayer.shape,
        position: this.cachedState?.position || [pos.x, pos.y, pos.z],
        rotation: this.cachedState?.rotation || [0, rot, 0],
        isMoving: state.isLocalPlayerMoving,
        isMicOn: state.micEnabled,
        isVideoOn: state.videoEnabled, // New flag
  ...
  addStream(stream: MediaStream, type: 'audio' | 'screen' | 'video') {
      if(this.room) {
