...
// Updated App ID to ensure fresh connections and fix sync
const APP_ID = 'room8_v9_final_fix'; 
...
class NetworkService {
  ...
  connect(roomId: string) {
    ...
    // CRITICAL FIX: Normalize Room ID. 
    // This prevents user "A" from being in room "abc" and user "B" in room "ABC".
    // Now everyone is forced into the same uppercase, trimmed namespace.
    const cleanId = roomId.trim().toUpperCase();

    console.log(`[Network] Connecting to room: ${cleanId}`);
    ...
    this.heartbeatInterval = setInterval(() => {
        this.broadcastMyState(true);
    }, 1000); // Faster heartbeat for snappier entry
  }
