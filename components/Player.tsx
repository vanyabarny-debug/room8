...
const RemoteAudioController: React.FC<{ stream: MediaStream | undefined, position: [number, number, number] }> = ({ stream, position }) => {
    ...
    useFrame(() => {
        if (audioRef.current) {
            // Check if this stream is currently open in Cinema Mode
            if (activeCinemaStream && stream && activeCinemaStream.id === stream.id) {
                // If in Cinema Mode, mute the 3D spatial audio
                // We let the Modal Video Player handle the audio (full quality, controllable)
...
export const RemotePlayer: React.FC<{ id: string; data: any }> = ({ id, data }) => {
  ...
  return (
    <group ref={group} position={data.position} onClick={(e) => { e.stopPropagation(); setShowMenu(!showMenu); setShowRemoveConfirm(false); }}>
      ...
      {/* If screen stream has audio tracks, play them spatially or handle cinema mode handoff */}
...
export const LocalPlayer = () => {
  ...
  useFrame((state) => {
    ...
    if (isJoystickActive && group.current) {
        ...
        // Y=0.8 means camera is near the feet/waist. 
        // Z=3.5 means it's close to the character.
        const offset = new THREE.Vector3(0, 0.8, 3.5); 
        ...
        // Look way UP above the head (Y + 4.0)
        state.camera.lookAt(group.current.position.x, group.current.position.y + 4.0, group.current.position.z);
    } 
