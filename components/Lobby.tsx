...
export const Lobby = () => {
  ...
  const noScrollClass = "[&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']";
  ...
  const renderInfoMenu = () => (
      <div className={`absolute bottom-6 right-6 z-[60] flex-col items-end gap-3 ${isWorldListOpen ? 'hidden md:flex' : 'flex'}`}>
          <div 
  ...
  const renderLegalModal = () => (
      showLegalModal && (
        <div className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-6" onClick={() => setShowLegalModal(null)}>
            <div className={`${cardBgClass} border ${borderClass} rounded-xl p-8 max-w-sm w-full shadow-2xl animate-bounce-in text-left relative ${noScrollClass} overflow-y-auto max-h-[85vh]`} onClick={e => e.stopPropagation()}>
  ...
  return (
    <div className={`absolute inset-0 flex flex-col overflow-hidden font-sans ${bgClass} ${textClass}`}>
      ...
      <div className={`absolute top-0 left-0 right-0 p-6 z-[60] flex items-center justify-between transition-colors duration-500 pointer-events-none ${selectedRoomId ? 'bg-black/60 backdrop-blur-md border-b border-white/5' : ''}`}>
          
          {/* LEFT: LOGO */}
          <div className="flex-shrink-0 z-20 pointer-events-auto">
            {selectedRoomId ? (
               <button 
                    onClick={() => {
                        if (selectedRoomId) {
                ...
                >
                    {/* Desktop: Logo */}
                    <h1 className="hidden md:block text-3xl md:text-4xl font-black tracking-tighter leading-none drop-shadow-[0_4px_6px_rgba(0,0,0,0.8)] font-sans select-none text-white">
          ...
          {selectedRoomId && (
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg px-4 hidden md:block z-10 pointer-events-auto">
          ...
          <div className="flex-shrink-0 z-20 pointer-events-auto">
             ...
             <div className="md:hidden relative">
                <button onClick={() => setShowMobileMenu(!showMobileMenu)} className={`${getGlassBtnClass(showMobileMenu)} w-[50px]`}>
                ...
                {showMobileMenu && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-black/90 backdrop-blur-xl border border-white/20 rounded-xl p-2 flex flex-col gap-2 shadow-2xl animate-fade-in z-50">
                        <button onClick={() => { setShowFriends(!showFriends); setShowMobileMenu(false); }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 text-white transition text-sm font-bold">
                            <UserGroupIcon className="w-5 h-5 text-blue-400" /><span>{t('friends_list')}</span>
                        </button>
                        <button onClick={() => { setShowProfile(!showProfile); setShowMobileMenu(false); }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 text-white transition text-sm font-bold">
                            <UserIcon className="w-5 h-5 text-purple-400" /><span>{t('profile_settings')}</span>
                        </button>
                        <button onClick={() => { setShowSettings(!showSettings); setShowMobileMenu(false); }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 text-white transition text-sm font-bold">
                            <Cog6ToothIcon className="w-5 h-5 text-gray-400" /><span>{t('settings_title')}</span>
                        </button>
                    </div>
                )}
             </div>
      ...
      {/* CENTER STAGE (Main Input) */}
      <div 
        className={`absolute z-30 transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] flex items-center justify-center pointer-events-none
        ${isWorldListOpen 
            // ALIGNMENT FIX: 
            // Left: 24px (6) to match Logo
            // Right: 24px (6) to match end of burger menu button (stretch fully)
            // Desktop: Centered as normal
            ? 'top-24 left-6 right-6 md:top-6 md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-full md:max-w-xl md:px-6' 
            : 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg px-6'}
        ${selectedRoomId ? 'opacity-0 translate-y-20' : 'opacity-100'}
        `}
      >
          {/* Increased gap from 2 to 4 */}
          <div className={`relative flex items-center transition-all duration-500 w-full pointer-events-auto gap-4 md:gap-2`}>
                {isWorldListOpen && (
                    <>
                    {/* Magnifier Moved to LEFT for better alignment */}
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 z-20 text-gray-500 pointer-events-none">
                        <MagnifyingGlassIcon className="w-5 h-5" />
                    </div>
                    {/* Sort/Filter Icons stay on RIGHT */}
                ...
                <input 
                    ...
                    // Adjusted padding: pl-10 for left icon, pr-20 for right icons
                    className={`${inputStyle} w-full transition-all duration-500 ${isWorldListOpen ? 'pl-10 pr-20' : ''} text-lg`}
                />
                ...
                {/* Sort/Filter Menus (Same as before) */}
                {isWorldListOpen && showSortMenu && (
                    <div className="absolute top-14 right-0 bg-black/90 backdrop-blur-xl border border-white/20 rounded-xl p-2 w-48 shadow-2xl z-50 animate-fade-in flex flex-col gap-1 pointer-events-auto">
                        <div className="text-[10px] text-gray-500 uppercase font-bold px-2 py-1">{t('sort_title')}</div>
                        {[
                            { id: 'popular', label: t('sort_popular') },
                            { id: 'newest', label: t('sort_newest') },
                            { id: 'online_desc', label: t('sort_online_desc') },
                            { id: 'online_asc', label: t('sort_online_asc') }
                        ].map(opt => (
                            <button 
                                key={opt.id}
                                onClick={() => { setSortBy(opt.id as any); setShowSortMenu(false); }}
                                className={`text-left px-3 py-2 rounded-lg text-sm transition ${sortBy === opt.id ? 'bg-blue-600 text-white font-bold' : 'text-gray-300 hover:bg-white/10'}`}
                            >
                                {opt.label}
                            </button>
                        ))}
                ...
                {isWorldListOpen && showFilterMenu && (
                    <div className="absolute top-14 right-0 bg-black/90 backdrop-blur-xl border border-white/20 rounded-xl p-4 w-64 shadow-2xl z-50 animate-fade-in pointer-events-auto">
                        {/* ... Filter content ... */}
                        <div className="flex justify-between items-center mb-4">
                            <span className="text-xs text-gray-500 uppercase font-bold">{t('filter_title')}</span>
                            {(filterMinPlayers > 0 || filterType !== 'all' || Object.values(filterRules).some(Boolean)) && (
                                <button onClick={() => { setFilterMinPlayers(0); setFilterType('all'); setFilterRules({screenShare:false, micMute:false, ignore:false}); }} className="text-[10px] text-red-400 hover:text-white">Reset</button>
                            )}
                        </div>
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between text-xs mb-1">
                                    <span className="text-gray-400">{t('filter_min_players')}</span>
                                    <span className="font-mono">{filterMinPlayers}</span>
                                </div>
                                <input 
                                    type="range" min="0" max="50" step="1" 
                                    value={filterMinPlayers} 
                                    onChange={(e) => setFilterMinPlayers(parseInt(e.target.value))}
                                    className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-blue-500"
                                />
                            </div>
                            <div>
                                <div className="text-xs text-gray-400 mb-2">{t('location')}</div>
                                <div className="flex flex-wrap gap-2">
                                    {['all', 'day', 'night', 'office'].map(type => (
                                        <button 
                                            key={type}
                                            onClick={() => setFilterType(type as any)}
                                            className={`px-2 py-1 rounded text-[10px] uppercase border transition ${filterType === type ? 'bg-white text-black border-white font-bold' : 'border-white/20 text-gray-400 hover:border-white'}`}
                                        >
                            ...
                            <div>
                                <div className="text-xs text-gray-400 mb-2">{t('filter_rules')}</div>
                                <div className="flex flex-col gap-2">
                                    <label className="flex items-center gap-2 cursor-pointer group">
                                        <div className={`w-4 h-4 border rounded flex items-center justify-center transition ${filterRules.screenShare ? 'bg-blue-600 border-blue-600' : 'border-white/30 group-hover:border-white'}`}>
                                            {filterRules.screenShare && <CheckIcon className="w-3 h-3 text-white" />}
                                        </div>
                                        <input type="checkbox" className="hidden" checked={filterRules.screenShare} onChange={e => setFilterRules({...filterRules, screenShare: e.target.checked})} />
                                        <span className={`text-xs ${filterRules.screenShare ? 'text-white' : 'text-gray-500'}`}>{t('rule_screen')}</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer group">
                                        <div className={`w-4 h-4 border rounded flex items-center justify-center transition ${filterRules.micMute ? 'bg-blue-600 border-blue-600' : 'border-white/30 group-hover:border-white'}`}>
                                            {filterRules.micMute && <CheckIcon className="w-3 h-3 text-white" />}
                                        </div>
                                        <input type="checkbox" className="hidden" checked={filterRules.micMute} onChange={e => setFilterRules({...filterRules, micMute: e.target.checked})} />
                                        <span className={`text-xs ${filterRules.micMute ? 'text-white' : 'text-gray-500'}`}>{t('rule_mic')}</span>
                                    </label>
                                     <label className="flex items-center gap-2 cursor-pointer group">
                                        <div className={`w-4 h-4 border rounded flex items-center justify-center transition ${filterRules.ignore ? 'bg-blue-600 border-blue-600' : 'border-white/30 group-hover:border-white'}`}>
                                            {filterRules.ignore && <CheckIcon className="w-3 h-3 text-white" />}
                                        </div>
                                        <input type="checkbox" className="hidden" checked={filterRules.ignore} onChange={e => setFilterRules({...filterRules, ignore: e.target.checked})} />
                                        <span className={`text-xs ${filterRules.ignore ? 'text-white' : 'text-gray-500'}`}>{t('rule_ignore')}</span>
                                    </label>
                                </div>
                            </div>
                        </div>
      ...
      {selectedRoomId && selectedRoom && (
        <div className="fixed inset-0 z-30 bg-black md:bg-black/60 md:backdrop-blur-xl overflow-hidden animate-fade-in">
            
            {/* Floating Back Button - FIXED POSITION (Outside Scroll) - Desktop Only */}
            <div className="hidden md:block absolute top-24 left-6 z-[70] pointer-events-auto">
                <button onClick={() => setSelectedRoomId(null)} className="p-2 text-white/50 hover:text-white hover:-translate-x-1 transition-all bg-black/20 rounded-full backdrop-blur-sm border border-white/5">
                    <ArrowLeftIcon className="w-8 h-8"/>
                </button>
            </div>

            {/* Scroll Container - Removed scrollbar visually on mobile */}
            <div 
                ref={roomScrollRef}
                onScroll={handleRoomScroll}
                className={`h-full overflow-y-auto md:custom-scroll ${noScrollClass} pt-24 md:pt-24 relative`}
            >
                <div className="w-full h-full md:max-w-2xl md:mx-auto md:px-4 md:py-6 md:pb-20 p-0">
                    {/* ... (Rest of Room Card content remains the same) ... */}
                    <div className="bg-black md:border md:border-white/20 md:rounded-xl overflow-hidden relative shadow-2xl min-h-screen md:min-h-0 border-none rounded-none">
                        {/* Banner */}
                        ...
                        <div className="p-6 pt-2">
                            {/* ... (Room details header, stats, description, rules, etc. - No changes needed inside) ... */}
                            ...
                            {/* Description/Edit Form (Shortened for brevity - logic remains) */}
                            <div className="mb-6">
                                {editMode ? (
                                    <div className="space-y-4">
                                        ...
                                        <div className="border border-white/10 rounded-xl p-4 bg-white/5">
                                            ...
                                            <div className="mt-4">
                                                ...
                                                <div className="flex gap-2 mb-2">
                                                    <input value={newEditRuleText} onChange={e => setNewEditRuleText(e.target.value)} className="flex-1 bg-black border border-white/20 rounded-lg px-2 py-1 text-sm" placeholder="Add rule..." />
                                                    <button onClick={() => setNewCustomRuleType(newCustomRuleType === 'allow' ? 'forbid' : 'allow')} className={`px-2 py-1 rounded text-xs font-bold ${newCustomRuleType === 'allow' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{newCustomRuleType === 'allow' ? 'Allow' : 'Forbid'}</button>
                                                    <button onClick={handleAddCustomRule} className="bg-white/10 p-1 rounded-lg"><PlusIcon className="w-5 h-5"/></button>
                                                </div>
                                                ...
                                                {isEmojiPickerOpen && (
                                                    <div className="bg-black border border-white/20 rounded-lg p-2 grid grid-cols-8 gap-1 h-32 overflow-y-auto custom-scroll mb-2">
                                                        {AVAILABLE_EMOJIS.map(e => (
                                                            <button key={e} onClick={() => handleToggleEditEmoji(e)} className={`w-8 h-8 flex items-center justify-center rounded hover:bg-white/20 ${selectedEmojis.includes(e) ? 'bg-blue-600' : ''}`}>{e}</button>
                                                        ))}
                                                    </div>
                                                )}
                                            </div>
                            ...
                            {/* Events Section (Truncated for brevity, logic identical) */}
                            <div className="border-t border-white/10 pt-8">
                                <div className="flex items-center gap-2 mb-6"><h2 className="text-xl font-bold text-white">Events</h2></div>
                                {selectedRoom.creatorId === localPlayer.id && (
                                    <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-8 transition-all">
                                        {!isEventFormExpanded ? (
                                            <input className="w-full bg-transparent border-none outline-none text-white placeholder:text-gray-500 text-base" placeholder="Tell about an event..." onFocus={() => setIsEventFormExpanded(true)} />
                                        ) : (
                                            <div className="animate-fade-in">
                                                <input value={newPostTitle} onChange={(e) => setNewPostTitle(e.target.value)} className="w-full bg-transparent border-b border-white/10 outline-none text-white placeholder:text-gray-500 text-lg font-bold mb-3 pb-2" placeholder="Event Title" autoFocus />
                                                <input type="date" value={newPostDate} onChange={(e) => setNewPostDate(e.target.value)} className="w-full bg-transparent border-b border-white/10 outline-none text-gray-400 text-sm mb-3 pb-2" />
                                                <textarea value={newPostContent} onChange={(e) => setNewPostContent(e.target.value)} className="w-full bg-transparent border-none outline-none text-white placeholder:text-gray-500 text-sm resize-none mb-3" placeholder="Description..." rows={3} />
                                                {newPostImage && (
                                                    <div className="relative mb-3"><img src={newPostImage} alt="Preview" className="max-h-40 rounded-lg border border-white/10 object-contain bg-black/50" /><button onClick={() => setNewPostImage(null)} className="absolute top-1 right-1 bg-red-600 rounded-full p-1"><XMarkIcon className="w-4 h-4 text-white"/></button></div>
                                                )}
                                                {newPostType === 'poll' && (
                                                    <div className="bg-black/30 p-3 rounded-lg mb-3">
                                                        ...
                                                        {pollOptions.map((opt, idx) => (
                                                            <div key={opt.id} className="flex gap-2 mb-2"><input value={opt.text} onChange={(e) => { const newOpts = [...pollOptions]; newOpts[idx].text = e.target.value; setPollOptions(newOpts); }} className="flex-1 bg-black border border-white/20 rounded-lg px-2 py-1 text-sm" placeholder={`Option ${idx + 1}`} />{pollOptions.length > 2 && <button onClick={() => setPollOptions(pollOptions.filter(o => o.id !== opt.id))}><XMarkIcon className="w-4 h-4 text-red-500"/></button>}</div>
                                                        ))}
                                                ...
                                                <div className="flex justify-between items-center border-t border-white/10 pt-3">
                                                    <div className="flex gap-2"><input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileSelect} /><button onClick={() => fileInputRef.current?.click()} className={`p-2 rounded-xl hover:bg-white/10 transition ${newPostImage ? 'text-blue-400 bg-blue-400/10' : 'text-gray-400'}`}><PhotoIcon className="w-5 h-5"/></button><button onClick={() => setNewPostType(newPostType === 'poll' ? 'text' : 'poll')} className={`p-2 rounded-xl hover:bg-white/10 transition ${newPostType==='poll' ? 'text-blue-400 bg-blue-400/10' : 'text-gray-400'}`}><ListBulletIcon className="w-5 h-5"/></button></div>
                                                    <div className="flex gap-2"><button onClick={() => setIsEventFormExpanded(false)} className="px-4 py-2 text-gray-400 hover:text-white text-sm font-bold">Cancel</button><button onClick={handleCreatePost} disabled={!newPostTitle.trim()} className="px-6 py-2 bg-white text-black rounded-xl text-sm font-bold hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition">Post</button></div>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="bg-black border border-white/20 rounded-xl overflow-hidden">
                                    {selectedRoom.posts?.map((post) => (
                                        <div key={post.id} className="border-b border-white/10 last:border-0 hover:bg-white/5 transition relative group">
                                            <div className="p-5">
                                                <div className="flex justify-between items-start mb-2"><div><h3 className="text-xl font-bold text-white mb-1">{post.title}</h3>{post.eventDate && <div className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">{new Date(post.eventDate).toLocaleDateString()}</div>}</div></div>
                                                <div className="cursor-default">
                                                    <p className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap mb-4">{post.content}</p>
                                                    {post.imageUrl && (<div className="w-full mb-4 cursor-pointer overflow-hidden rounded-lg border border-white/10" onClick={() => setLightboxImage(post.imageUrl!)}><img src={post.imageUrl} className="w-full h-auto object-contain max-h-[500px] bg-black/50" alt="Event" /></div>)}
                                                    {post.type === 'poll' && post.pollOptions && (
                                                        <div className="space-y-2 mb-4 bg-white/5 p-3 rounded-xl border border-white/5">
                                                            {post.pollOptions.map(opt => { const totalVotes = post.pollOptions!.reduce((acc, o) => acc + o.votes.length, 0); const percent = totalVotes > 0 ? (opt.votes.length / totalVotes) * 100 : 0; const iVoted = opt.votes.includes(localPlayer.id); return (<button key={opt.id} onClick={(e) => { e.stopPropagation(); votePoll(selectedRoom.id, post.id, opt.id); }} className={`w-full relative h-10 rounded-lg overflow-hidden border transition ${iVoted ? 'border-blue-500' : 'border-white/10 hover:border-white/30'}`}><div className="absolute top-0 left-0 bottom-0 bg-white/10 transition-all duration-500" style={{ width: `${percent}%` }} /><div className="absolute inset-0 flex justify-between items-center px-3 z-10 text-sm"><span>{opt.text}</span><span className="font-mono text-xs opacity-70">{Math.round(percent)}%</span></div></button>) })}
                                                ...
                                                <div className="flex items-center justify-between pt-2 mt-2">
                                                    <div className="flex gap-2 overflow-x-auto no-scrollbar"><button onClick={() => togglePostGoing(selectedRoom.id, post.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition border ${post.going.includes(localPlayer.id) ? 'bg-green-600 border-green-600 text-white' : 'bg-white/5 border-transparent hover:bg-white/10 text-gray-300'}`}>{post.going.includes(localPlayer.id) ? <BellSolid className="w-4 h-4"/> : <BellOutline className="w-4 h-4"/>}<span>{post.going.length || 'Going'}</span></button><div className="w-px h-8 bg-white/10 mx-1"/>{(selectedRoom.allowedReactions || DEFAULT_REACTIONS).map(emoji => { const count = (post.reactions?.[emoji] || []).length; const iReacted = (post.reactions?.[emoji] || []).includes(localPlayer.id); return (<button key={emoji} onClick={() => togglePostReaction(selectedRoom.id, post.id, emoji)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm transition border ${iReacted ? 'bg-blue-900/30 border-blue-500/50 text-blue-200' : 'bg-white/5 border-transparent hover:bg-white/10 text-gray-400'}`}><span>{emoji}</span>{count > 0 && <span className="text-xs">{count}</span>}</button>) })}</div>
                                    ...
                                    {(!selectedRoom.posts || selectedRoom.posts.length === 0) && (<div className="text-center py-12 text-gray-600"><p className="text-sm">No events yet.</p></div>)}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {showRoomScrollTop && (
                    <div className="sticky bottom-6 left-0 right-0 flex justify-center z-50 pointer-events-none"><button onClick={scrollToTopRoom} className="pointer-events-auto bg-black/80 backdrop-blur border border-white/20 text-white rounded-full p-3 shadow-lg hover:bg-white hover:text-black transition animate-bounce-in"><ArrowUpIcon className="w-6 h-6" /></button></div>
                )}
            </div>
        </div>
      )}

      {/* Lightbox, Share Modal, Friends Panel, Menu, Legal Modal (Unchanged) */}
      {lightboxImage && (<div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4" onClick={() => setLightboxImage(null)}><button className="absolute top-6 right-6 text-white p-2 bg-white/10 rounded-full hover:bg-white/20"><XMarkIcon className="w-8 h-8"/></button><img src={lightboxImage} alt="Full view" className="max-w-full max-h-full object-contain shadow-2xl" onClick={e => e.stopPropagation()}/></div>)}
      {showShareModal && (<div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur flex items-center justify-center" onClick={() => setShowShareModal(null)}><div className="bg-black border border-white/20 p-8 rounded-2xl animate-bounce-in text-center max-w-sm w-full mx-4 shadow-2xl" onClick={e => e.stopPropagation()}><h3 className="text-xl font-bold mb-6 text-white">Share Event</h3><div className="flex gap-8 justify-center"><button onClick={() => handleSharePost(showShareModal)} className="flex flex-col items-center gap-3 group"><div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center group-hover:bg-white/20 group-hover:scale-110 transition"><LinkIcon className="w-6 h-6 text-white"/></div><span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Copy Link</span></button><button onClick={() => { handleSharePost(showShareModal); }} className="flex flex-col items-center gap-3 group"><div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center group-hover:bg-blue-600/20 group-hover:border-blue-500 group-hover:scale-110 transition"><GlobeAltIcon className="w-6 h-6 text-white"/></div><span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Socials</span></button></div></div></div>)}
      {showFriends && (
          <div className={`absolute top-24 right-6 w-auto max-w-[90vw] ${cardBgClass} border ${borderClass} rounded-xl p-4 flex flex-col items-center origin-top-right animate-bounce-in z-[100] shadow-2xl relative`}>
              <button onClick={() => setShowFriends(false)} className="absolute top-2 right-2 text-gray-500 hover:text-white p-1">
                  <XMarkIcon className="w-4 h-4" />
              </button>
              <div className="mt-2">
                  {friends.length === 0 ? <div className={`${secondaryTextClass} text-sm`}>{t('friends_empty')}</div> : (<div className="flex gap-4 overflow-x-auto pb-2 w-full justify-center">{friends.map(fid => (<div key={fid} className={`${cardBgClass} border ${borderClass} px-4 py-2 rounded-xl flex items-center gap-2 whitespace-nowrap`}><div className="w-2 h-2 rounded-full bg-green-500" /><span className="text-sm font-mono">{fid.substring(0,8)}</span></div>))}</div>)}
              </div>
          </div>
      )}
      {renderInfoMenu()}
      ...
      {/* Create Room Modal - Fixed Z-index */}
      {showCreateModal && (
          <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
              <div className={`${cardBgClass} border ${borderClass} rounded-xl w-full max-w-sm p-6 relative flex flex-col animate-bounce-in max-h-[90vh] overflow-y-auto custom-scroll shadow-2xl ${noScrollClass}`}>
                   <div className="flex items-center justify-between mb-6"><h2 className="text-2xl font-bold">{t('create_title')}</h2><button onClick={() => setShowCreateModal(false)} className="hover:opacity-70 p-1"><XMarkIcon className="w-6 h-6"/></button></div>
                   <div className="space-y-5">
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-1 block">{t('room_name')}</label><input value={roomName} onChange={(e) => setRoomName(e.target.value)} className={`${inputStyle} w-full text-sm`} placeholder={t('room_name')} autoFocus /></div>
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-1 block">{t('room_desc')}</label><textarea value={roomDesc} onChange={(e) => setRoomDesc(e.target.value)} className="w-full bg-black border-2 border-white/20 rounded-xl text-white px-4 py-3 outline-none focus:border-white/40 resize-none h-20 text-sm" placeholder={t('room_desc')} /></div>
                       <div><div className="flex items-center justify-between mb-2"><label className="text-xs uppercase text-gray-500 font-bold block">{t('access')}</label><div className="text-xs font-bold text-blue-400">{isPrivate ? t('private') : t('public')}</div></div><div className="flex items-center gap-2"><div className={`flex-1 flex items-center bg-black border ${idError ? 'border-red-500' : 'border-white/20'} rounded-lg px-2 h-10 transition-all`}><span className="text-gray-500 text-xs">@</span><input value={draftRoomId} onChange={(e) => checkIdAvailability(e.target.value.toUpperCase())} className="bg-transparent border-none outline-none text-white text-xs w-full ml-1 font-mono uppercase" placeholder="HANDLE" />{idError && <ExclamationCircleIcon className="w-4 h-4 text-red-500" />}</div><Toggle label="" checked={isPrivate} onChange={setIsPrivate} /></div></div>
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-1 block">{t('location')}</label><div className="flex gap-2">{['day', 'night', 'office'].map(type => (<button key={type} onClick={() => setRoomType(type as any)} className={`flex-1 py-2 text-sm rounded-lg border transition capitalize ${roomType === type ? 'bg-blue-600 text-white border-blue-500 font-bold' : 'border-white/20 text-gray-400 hover:text-white'}`}>{t(type as any)}</button>))}</div></div>
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-2 block">{t('rules')}</label><Toggle label={t('rule_screen')} checked={allowScreenShare} onChange={setAllowScreenShare} /><Toggle label={t('rule_mic')} checked={forbidMicMute} onChange={setForbidMicMute} /><Toggle label={t('rule_ignore')} checked={preventIgnoring} onChange={setPreventIgnoring} /></div>
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-2 block">{t('custom_rules')}</label><div className="flex gap-2 mb-2"><input value={newCustomRuleText} onChange={(e) => setNewCustomRuleText(e.target.value)} className="flex-1 bg-black border border-white/20 rounded-lg px-3 py-2 text-xs text-white outline-none focus:border-white/40" placeholder={t('add_rule_placeholder')} onKeyDown={(e) => e.key === 'Enter' && handleAddCustomRule()} /><button onClick={() => setNewCustomRuleType(newCustomRuleType === 'allow' ? 'forbid' : 'allow')} className={`px-2 py-1 rounded text-xs font-bold ${newCustomRuleType === 'allow' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{newCustomRuleType === 'allow' ? 'Allow' : 'Forbid'}</button><button onClick={handleAddCustomRule} className="bg-white/10 hover:bg-white/20 px-3 py-2 rounded-lg text-white transition"><PlusIcon className="w-4 h-4" /></button></div>{customRules.length > 0 && (<div className="flex flex-col gap-1 max-h-24 overflow-y-auto custom-scroll">{customRules.map((rule, i) => (<div key={i} className="flex justify-between items-center bg-white/5 px-2 py-1.5 rounded border border-white/5"><span className={`text-xs ${rule.type === 'allow' ? 'text-green-400' : 'text-red-400'} truncate`}>[{rule.type.toUpperCase()}] {rule.text}</span><button onClick={() => handleRemoveCustomRule(i)} className="text-red-400 hover:text-red-300"><XMarkIcon className="w-3 h-3"/></button></div>))}</div>)}</div>
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-2 block">{t('reactions')}</label><div className="flex items-center gap-2 mb-2 flex-wrap">{selectedEmojis.map(emoji => (<button key={emoji} onClick={() => handleToggleEmoji(emoji)} className="w-8 h-8 flex items-center justify-center bg-white/10 rounded-full hover:bg-red-500/20">{emoji}</button>))}{selectedEmojis.length < 5 && (<button onClick={() => setIsEmojiPickerOpen(!isEmojiPickerOpen)} className="w-8 h-8 flex items-center justify-center border border-dashed border-white/30 rounded-full text-gray-400 hover:text-white"><PlusIcon className="w-4 h-4"/></button>)}</div>{isEmojiPickerOpen && (<div className="bg-black border border-white/20 rounded-lg p-2 grid grid-cols-8 gap-1 h-32 overflow-y-auto custom-scroll mb-2">{AVAILABLE_EMOJIS.map(e => (<button key={e} onClick={() => handleToggleEmoji(e)} className={`w-8 h-8 flex items-center justify-center rounded hover:bg-white/20 ${selectedEmojis.includes(e) ? 'bg-blue-600' : ''}`}>{e}</button>))}</div>)}</div>
                       <div><label className="text-xs uppercase text-gray-500 font-bold mb-2 block">Header Gradient</label><div className="flex items-center gap-2 mb-2">{gradientColors.map((c, i) => (<div key={i} className="relative group"><input type="color" value={c} onChange={(e) => handleUpdateColor(i, e.target.value)} className="w-8 h-8 rounded-full cursor-pointer border-none bg-transparent" />{gradientColors.length > 2 && <button onClick={() => handleRemoveColor(i)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-3 h-3 flex items-center justify-center text-[8px] opacity-0 group-hover:opacity-100"><XMarkIcon/></button>}</div>))}{gradientColors.length < 5 && <button onClick={handleAddColor} className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white/50 hover:text-white hover:border-white"><PlusIcon className="w-4 h-4"/></button>}</div><div className="w-full h-8 rounded-lg" style={{ background: getGradientString() }} /></div>
                       <div className="pt-2"><button onClick={handleCreate} className="w-full py-4 bg-white text-black rounded-xl font-bold text-lg hover:scale-[1.02] active:scale-[0.98] transition shadow-lg flex items-center justify-center gap-2"><PlusIcon className="w-5 h-5" /><span>{t('create_room')}</span></button></div>
                   </div>
              </div>
          </div>
      )}

      {/* World List Drawer - Adjusted mobile vertical position for better balance */}
      <div 
        ref={worldListRef}
        className={`fixed left-0 right-0 bottom-0 z-20 transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] flex flex-col items-center
            ${isWorldListOpen 
                // Moved down to top-40 on mobile to clear search bar cleanly
                ? 'top-40 md:top-24 rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] bg-black/90 backdrop-blur-3xl' 
      ...
      {/* Profile & Settings Modals - Fixed Z-Index to be above header */}
      {showProfile && (
        <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
             <div className={`${cardBgClass} border ${borderClass} rounded-xl w-full max-w-sm p-8 relative flex flex-col animate-bounce-in max-h-[90vh] overflow-y-auto custom-scroll shadow-2xl ${noScrollClass}`}>
      ...
      {showSettings && (
          <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
              <div className={`${cardBgClass} border ${borderClass} rounded-xl w-full max-w-sm p-8 relative flex flex-col animate-bounce-in shadow-2xl max-h-[90vh] overflow-y-auto custom-scroll ${noScrollClass}`}>
