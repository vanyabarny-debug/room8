...
export const Lobby = () => {
  ...
  const handleJoin = (id: string) => {
    ...
    // Try to find case-insensitive match in existing rooms
    const foundRoom = (Object.values(rooms) as RoomConfig[]).find(r => r.id.toLowerCase() === lowerId.toLowerCase());

    if (!foundRoom) {
        // Create a temporary local config for this private/unknown room
        // IMPORTANT: Use the input ID as-is (but trimmed) for consistency if creating new
        // OR normalize to uppercase if that's the convention. 
        // For custom rooms generated by generateRoomId, they are UPPERCASE.
        // Let's assume custom codes are case-insensitive and normalize to uppercase for the network ID if not found.
