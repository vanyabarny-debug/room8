...
export const Lobby = () => {
  ...
  const handleCreate = () => {
    ...
    const newRoom: RoomConfig = {
      ...
      type: roomType,
      region: language, // Tie room to current language server
      isPrivate: isPrivate,
      isOfficial: false,
  ...
  // Special handler for creating the private office from the template
  ...
  if (!hasOnboarded) {
    return (
      <div className="absolute inset-0 bg-black text-white flex flex-col items-center justify-center p-6 z-50">
        ...
        {/* Legal Modal */}
        {showLegalModal && (
            <div className="absolute inset-0 z-[60] bg-black/95 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in">
                <div className="bg-black border border-white/30 rounded-lg max-w-lg w-full p-8 relative flex flex-col shadow-2xl">
                    <button 
                    ...
                    <button 
                        onClick={() => setShowLegalModal(null)}
                        className="mt-8 bg-white text-black font-bold py-3 rounded-md hover:bg-gray-200 transition"
                    >
                        OK
                    </button>
                </div>
        ...
        <div className="w-full max-w-sm text-center animate-fade-in flex flex-col items-center">
          ...
          <div className="mt-8 text-[10px] text-gray-500 text-center max-w-xs leading-relaxed opacity-70 hover:opacity-100 transition-opacity">
              <span>{t('legal_agree')} </span>
              <button onClick={() => setShowLegalModal('terms')} className="underline hover:text-white transition decoration-white/50 hover:decoration-white">{t('legal_terms')}</button>
              <span> {t('legal_and')} </span>
              <button onClick={() => setShowLegalModal('privacy')} className="underline hover:text-white transition decoration-white/50 hover:decoration-white">{t('legal_privacy')}</button>.
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="absolute inset-0 bg-black text-white flex flex-col overflow-hidden font-sans">
      ...
      {/* Profile Panel - FIXED SCROLL */}
      {showProfile && (
        <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-black border border-white/20 p-8 rounded-2xl w-full max-w-sm flex flex-col relative overflow-hidden backdrop-blur-md shadow-2xl max-h-[90vh] animate-bounce-in">
                 <button onClick={() => setShowProfile(false)} className="absolute top-4 right-4 text-gray-500 hover:text-white z-10"><XMarkIcon className="w-5 h-5" /></button>
                 
                 <div className="flex flex-col gap-6 w-full overflow-y-auto custom-scroll pr-1">
                     <h2 className="text-xl font-bold mb-2">{t('profile_settings')}</h2>
      ...
      {/* SETTINGS MODAL (Language + Devices) */}
      {showSettings && (
          <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-black border border-white rounded-lg w-full max-w-sm p-6 relative flex flex-col animate-bounce-in max-h-[90vh] overflow-y-auto custom-scroll">
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="pt-2 border-t border-white/10">
                         ...
                         <div 
                            className="w-full py-2 bg-blue-600/10 text-blue-400 border border-blue-600/30 rounded-md text-sm font-bold flex items-center justify-center gap-2 cursor-not-allowed opacity-70"
                         >
      ...
      <div className="flex-1 overflow-y-auto p-6 custom-scroll z-10 relative">
         <div className="max-w-2xl mx-auto space-y-4">
            ...
            {visibleRooms.map((room) => {
               ...
               if (room.type === 'day') gradient = "from-green-900/20 to-transparent";
               if (room.type === 'night') gradient = "from-blue-900/20 to-transparent";
               if (room.type === 'office') gradient = "from-indigo-900/20 to-transparent";
          ...
          <div className="absolute inset-0 z-[60] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
              <div className="bg-black border border-white rounded-lg w-full max-w-2xl overflow-hidden shadow-2xl animate-bounce-in relative">
                  ...
                  <div className={`h-32 w-full bg-gradient-to-r 
                    ${selectedRoom.type === 'day' ? 'from-green-900 to-blue-900' : 
                      selectedRoom.type === 'night' ? 'from-black to-indigo-900' : 'from-indigo-900 to-blue-800'}`} 
                  />
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                             <button
                                onClick={() => { audioSynth.playUiClick(); setRoomType('day'); }}
                                className={`p-3 border rounded-md text-sm transition ${btnClass} ${roomType === 'day' ? 'bg-white text-black border-white' : 'border-white/30 hover:border-white'}`}
                            >
                                {t('day')}
                            </button>
                             <button
                                onClick={() => { audioSynth.playUiClick(); setRoomType('night'); }}
                                className={`p-3 border rounded-md text-sm transition ${btnClass} ${roomType === 'night' ? 'bg-white text-black border-white' : 'border-white/30 hover:border-white'}`}
                            >
                                {t('night')}
                            </button>
                            <button
                                onClick={() => { audioSynth.playUiClick(); setRoomType('office'); }}
                                className={`p-3 border rounded-md text-sm transition ${btnClass} ${roomType === 'office' ? 'bg-white text-black border-white' : 'border-white/30 hover:border-white'}`}
                            >
                                {t('office')}
                            </button>
                        </div>
