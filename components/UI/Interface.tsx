...
import { MicrophoneIcon, SpeakerWaveIcon, ComputerDesktopIcon, ArrowRightOnRectangleIcon, XMarkIcon, CheckIcon, Cog6ToothIcon, EyeSlashIcon, ClipboardDocumentIcon, BookmarkIcon as BookmarkSolid, UserGroupIcon, UserIcon, Bars3Icon } from '@heroicons/react/24/solid';
...
const Joystick = ({ setControls }: { setControls: (c: { forward: number, turn: number }) => void }) => {
    ...
    const { setJoystickActive } = useStore();
    ...
    return (
        <div 
            className="absolute bottom-10 left-10 w-40 h-40 z-50 touch-none select-none hidden landscape:flex items-center justify-center"
...
export const Interface = () => {
  ...
  // Determine if the current cinema stream is local
  ...
  useEffect(() => {
    if (activeCinemaStream && cinemaVideoRef.current) {
        ...
        // CRITICAL FIX: Explicitly set mute state on the DOM element.
        if (isLocalCinemaStream) {
            vid.muted = true;
            vid.volume = 0; 
        } else {
            vid.muted = false;
  ...
  return (
    <>
      ...
      <div className="absolute top-4 md:top-6 left-0 right-0 px-3 md:px-6 pointer-events-none flex flex-row justify-between items-start z-[200] safe-top landscape:scale-90 landscape:origin-top">
          ...
          {room && (
              <div className="pointer-events-auto relative z-50">
                  <div className="flex items-center gap-3">
                       ...
                       >
                            {/* UPDATED LOGO FONT STYLE TO MATCH LOBBY and Font Requirements */}
                            <h1 className="text-4xl font-black tracking-tighter leading-none drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)] font-sans">
          ...
          {/* Top Right Buttons - Square Shape + Counter + Mobile Burger */}
          <div className="pointer-events-auto relative z-50 flex gap-2 items-center">
             ...
             <div className="md:hidden relative">
                <button 
                    ...
                    className={`p-3 rounded-xl transition border border-white/10 ${btnClass} h-[50px] w-[50px] ${showMobileMenu ? 'bg-white text-black' : 'bg-black/60 backdrop-blur-md hover:bg-white/20 text-white'}`}
                >
                ...
                {showMobileMenu && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-black/90 backdrop-blur-xl border border-white/20 rounded-xl p-2 flex flex-col gap-2 shadow-2xl animate-fade-in z-50">
                        <button onClick={() => { setShowFriendsList(!showFriendsList); setShowMobileMenu(false); }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 text-white transition text-sm font-bold relative">
                            <UserGroupIcon className="w-5 h-5 text-blue-400" />
                            <span>{t('friends_list')}</span>
                            {onlineFriends.length > 0 && <div className="w-2 h-2 bg-blue-500 rounded-full ml-auto" />}
                        </button>
                        <button onClick={() => { setShowSettingsModal(true); setShowMobileMenu(false); }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 text-white transition text-sm font-bold">
            ...
            {/* Online Counter Badge - Always Visible */}
      ...
      {activeCinemaStream && (
        <div className="absolute inset-0 z-[250] bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
            <button 
                onClick={() => setActiveCinemaStream(null)} 
            ...
            <div className="w-full max-w-7xl aspect-video bg-black rounded-2xl shadow-2xl overflow-hidden border border-gray-800 animate-bounce-in relative">
                {/* 
                    ...
                    Muted state is controlled via ref in useEffect to ensure local streams are silent.
                */}
                <video 
                    ref={cinemaVideoRef} 
                    autoPlay 
                    controls 
                    muted={isLocalCinemaStream} 
