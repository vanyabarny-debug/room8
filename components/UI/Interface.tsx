...
import { MicrophoneIcon, SpeakerWaveIcon, ComputerDesktopIcon, ArrowRightOnRectangleIcon, XMarkIcon, CheckIcon, Cog6ToothIcon, EyeSlashIcon, ClipboardDocumentIcon, BookmarkIcon as BookmarkSolid, UserGroupIcon, UserIcon, Bars3Icon, FaceSmileIcon, ArrowUpIcon, VideoCameraIcon, PhoneIcon, PlusIcon, LinkIcon, EllipsisHorizontalIcon, SpeakerXMarkIcon as SpeakerMuteIcon } from '@heroicons/react/24/solid';
import { MicrophoneIcon as MicOff, SpeakerXMarkIcon, ComputerDesktopIcon as ScreenOff, BookmarkIcon as BookmarkOutline, VideoCameraSlashIcon } from '@heroicons/react/24/outline';
import { AudioVisualizer } from './AudioVisualizer';
import { DEFAULT_REACTIONS, TRANSLATIONS, AVAILABLE_EMOJIS } from '../../constants';
...
const ControlBtn = ({ active, onClick, onTouchStart, onTouchEnd, icon: Icon, offIcon: OffIcon, label, activeColor = "text-white", customClass = "", extra, rotateIcon = 0 }: any) => {
  ...
  return (
    <div className="relative group flex flex-col items-center flex-shrink-0">
      <button
        ...
        // Unified Circular Style: w-12 h-12 rounded-full
        className={`w-12 h-12 md:w-14 md:h-14 rounded-full transition-all duration-200 border flex items-center justify-center
        ${active ? `bg-white/20 ${activeColor} border-white/40` : `bg-black/60 border-white/10 text-gray-300 hover:text-white hover:bg-white/10 ${customClass}`}
        hover:scale-105 active:scale-95 relative overflow-hidden backdrop-blur-md shadow-lg`}
        title={label}
      >
        <TheIcon 
            className={`w-5 h-5 md:w-6 md:h-6 transition-transform`} 
            style={{ transform: rotateIcon ? `rotate(${rotateIcon}deg)` : 'none' }}
        />
        {extra}
      </button>
    </div>
...
export const Interface = () => {
  const { 
    ...
    videoEnabled, toggleVideo, videoStream, // Added video props
    ...
    updateRoomDetails, globalMute, toggleGlobalMute
  } = useStore();
  ...
  const [showEmojiPicker, setShowEmojiPicker] = useState(false); // New for Call Mode
  
  const [zoneName, setZoneName] = useState<string | null>(null);
  const [devices, setDevices] = useState<MediaDeviceInfo[]>([]);
  const [copiedId, setCopiedId] = useState(false);
  
  const isUIOpen = showRoomInfo || showFriendsList || showSettingsModal || showMobileMenu || showEmojiPicker;
  ...
  const getRoomName = (room: RoomConfig) => {
    if (room.isOfficial) {
        ...
        if (room.type === 'call') return t('room_call_title');
    }
    return room.name;
  };
  const getRoomDesc = (room: RoomConfig) => {
    if (room.isOfficial) {
        ...
        if (room.type === 'call') return t('room_call_desc');
    }
  ...
  const handleUpdateReactions = (newEmoji: string) => {
      if (!room) return;
      let current = [...(room.allowedReactions || DEFAULT_REACTIONS)];
      if (!current.includes(newEmoji)) {
          // Replace the last one or push? Let's just keep 5 unique.
          // Shift out first, push new
          current.shift();
          current.push(newEmoji);
          updateRoomDetails(room.id, { allowedReactions: current });
      }
  ...
  // Push reactions down in standard view (Portrait)
  const shouldPushReactionsDown = isIdle && !isUIOpen && showReactions;

  return (
    <>
      {/* Disable mobile joystick in Call mode */}
      {!isCallMode && <MobileControls />}

      {/* ... (Friend Requests and TP Requests - Unchanged) ... */}
      
      {/* Top Bar Container - Now Hides on Idle */}
      <div className={`absolute top-4 md:top-6 left-0 right-0 px-3 md:px-6 pointer-events-none flex flex-row justify-between items-start z-[200] safe-top landscape:scale-90 landscape:origin-top transition-all duration-500 ${isIdle && !isUIOpen ? 'opacity-0 -translate-y-20' : 'opacity-100 translate-y-0'}`}>
          {room && (
          ...
          {/* Top Right Buttons - Hidden for Call Mode */}
          <div className="pointer-events-auto relative z-50 flex gap-2 items-start">
             ...
             <div className="hidden md:flex gap-2">
                {/* Friends & Counter: Hide in Call Mode */}
                {!isCallMode && (
                    <>
             ...
             {/* Mobile Menu Toggle - Minimalist 3 Dots */}
             <div className="md:hidden relative">
                <button onClick={() => setShowMobileMenu(!showMobileMenu)} className={`p-2 rounded-full transition text-white hover:bg-white/10 ${showMobileMenu ? 'bg-white/20' : ''}`}>
                ...
                {showMobileMenu && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-black/90 backdrop-blur-xl border border-white/20 rounded-xl p-2 flex flex-col gap-2 shadow-2xl animate-fade-in z-50">
                        {isCallMode && (
                            <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}?room=${activeRoomId}`); setShowMobileMenu(false); alert("Link copied!"); }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 text-white transition text-sm font-bold">
                                <LinkIcon className="w-5 h-5 text-green-400" /><span>Share Link</span>
                            </button>
                        )}
                        {!isCallMode && (
      ...
      <div className="absolute inset-0 pointer-events-none flex flex-col justify-end pb-6 md:pb-8 z-40 safe-bottom landscape:scale-90 landscape:origin-bottom">
        ...
        {/* Standard Reaction Bar (Portrait) - HIDDEN IN LANDSCAPE TO AVOID CLUTTER */}
        {room && showReactions && (
            <div 
                className={`flex justify-center mb-6 pointer-events-auto transition-transform duration-500 animate-bounce-in landscape:hidden`}
                style={{ transform: shouldPushReactionsDown ? 'translateY(60px)' : 'translateY(0)' }}
            >
                <div className="flex gap-3 bg-black/80 backdrop-blur-md px-4 py-3 rounded-full border border-white/20 shadow-2xl items-center relative">
                    ...
                    {/* Emoji Customization (Only in Call Mode) */}
                    {isCallMode && (
                        <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-gray-400 hover:text-white border border-white/10 ml-2">
                    ...
                    {showEmojiPicker && (
                        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-black/95 border border-white/20 p-2 rounded-xl grid grid-cols-6 gap-1 w-64 h-48 overflow-y-auto custom-scroll shadow-2xl z-[60]">
                            {AVAILABLE_EMOJIS.map(e => (
                                <button key={e} onClick={() => handleUpdateReactions(e)} className="w-8 h-8 text-xl flex items-center justify-center hover:bg-white/20 rounded">{e}</button>
                            ))}
                        </div>
                    )}
                </div>
        ...
        {/* VIGNETTE GRADIENT (Fades with Dock) */}
        <div 
            className={`absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-black/80 to-transparent transition-opacity duration-500 pointer-events-none z-[-1]
            ${isIdle && !isUIOpen ? 'opacity-0' : 'opacity-100'}`} 
        />

        {/* Main Dock - Responsive Container */}
        <div className={`flex justify-center pointer-events-auto w-full px-4 transition-all duration-500 ${isIdle && !isUIOpen ? 'opacity-0 translate-y-20 pointer-events-none' : 'opacity-100 translate-y-0'}`}>
          <div className="flex items-center gap-2 md:gap-3 px-2 py-2 rounded-full overflow-x-auto max-w-[95vw] md:max-w-full no-scrollbar scale-90 md:scale-100 origin-bottom">
            
            {/* Mic */}
            {!room?.rules.forbidMicMute && (
                <ControlBtn active={micEnabled} onClick={() => { audioSynth.playUiClick(); toggleMic(); }} icon={MicrophoneIcon} offIcon={MicOff} label={t('mic')} activeColor={micEnabled ? 'text-white' : 'bg-red-500/20 text-red-500'} customClass={!micEnabled ? 'bg-red-900/30 text-red-500' : ''} extra={micEnabled && <AudioVisualizer stream={micStream} active={micEnabled} className="absolute inset-0 opacity-20 pointer-events-none rounded-full" width={48} height={48} />} />
            )}

            {/* Video */}
            <ControlBtn active={videoEnabled} onClick={() => { audioSynth.playUiClick(); toggleVideo(); }} icon={VideoCameraIcon} offIcon={VideoCameraSlashIcon} label={t('video')} activeColor={videoEnabled ? 'text-white' : 'bg-red-500/20 text-red-500'} customClass={!videoEnabled ? 'bg-red-900/30 text-red-500' : ''} />
            
            {/* Global Speaker Toggle (Replaces simple audio toggle) */}
            <ControlBtn 
                active={!globalMute} 
                onClick={() => { audioSynth.playUiClick(); toggleGlobalMute(); }} 
                icon={SpeakerWaveIcon} 
                offIcon={SpeakerMuteIcon} 
                label="Toggle Speaker" 
                activeColor="text-white" 
                customClass={globalMute ? 'bg-gray-700 text-gray-400' : ''}
            />
            
            {/* Screen - HIDDEN ON MOBILE */}
            ...
            {/* Reactions Toggle (Integrated for Landscape Mode Compatibility) */}
            ...
            {/* Hangup Button - Rotated Phone Icon */}
            <div className="flex">
                 {isCallMode ? (
                     <ControlBtn 
                        active={false} 
                        ...
                        customClass="bg-red-600 text-white hover:bg-red-500 border-red-500 !w-14 !h-14"
                        rotateIcon={135} 
                     />
                 ) : (
