...
export const Interface = () => {
  ...
  useEffect(() => {
    if (activeCinemaStream && cinemaVideoRef.current) {
        ...
        // React props can sometimes be out of sync for media elements.
        // If it's my stream, I MUST mute it to prevent system audio feedback loop.
        if (isLocalCinemaStream) {
            vid.muted = true;
            vid.volume = 0; // Double safety
        } else {
            vid.muted = false;
            vid.volume = 1; // Full volume for remote streams
        }
