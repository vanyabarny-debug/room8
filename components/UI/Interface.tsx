...
const Joystick = ({ onMove, label, type }: { onMove: (val: number) => void, label: string, type: 'vertical' | 'horizontal' }) => {
    ...
    return (
        <div 
            className={`w-24 h-24 rounded-full border-2 ${active ? 'border-white bg-white/10 opacity-70' : 'border-white/20 bg-black/10 opacity-30'} flex items-center justify-center relative transition-all backdrop-blur-sm pointer-events-auto touch-none`}
            onTouchStart={handleStart}
            onTouchMove={handleMove}
            onTouchEnd={handleEnd}
        >
            <div 
                className={`w-8 h-8 rounded-full bg-white shadow-lg absolute transition-transform duration-75`}
...
export const Interface = () => {
  ...
  return (
    <>
      ...
      <div className={`absolute inset-0 z-30 pointer-events-none md:hidden flex flex-col justify-end pb-8 px-6 transition-opacity duration-1000 ${isIdle && !isUIOpen ? 'opacity-0' : 'opacity-100'}`}>
          <div className="flex justify-between w-full">
              <Joystick 
                type="vertical" 
                label="Move" 
                onMove={(val) => setControls({ forward: val })} 
              />
              <Joystick 
                type="horizontal" 
                label="Turn" 
                onMove={(val) => setControls({ turn: -val })}
              />
          </div>
      </div>
