...
export const isConferenceObstacle = (x: number, z: number) => {
    ...
    // Music Player Collision (New)
    // Positioned at x=16, z=60. Size approx 1.5x1.5
...
export const ConferenceScene = () => {
    ...
    const ceilingLights = useMemo(() => {
        return Array.from({ length: 4 }).map((_, i) => {
            ...
            return (
                <group key={z}>
                    {/* Main Light - Reduced Intensity (1.5 -> 0.8) */}
                    <pointLight position={[0, 14, z]} intensity={0.8} distance={40} color={conferenceState.lightColor} />
                    {/* Fake Fill Lights - Reduced Intensity (0.5 -> 0.25) */}
                    <pointLight position={[-12, 14, z]} intensity={0.25} distance={20} color={conferenceState.lightColor} />
                    <pointLight position={[12, 14, z]} intensity={0.25} distance={20} color={conferenceState.lightColor} />
                </group>
            );
        });
    ...
    return (
        <group>
            ...
            <pointLight position={[0, 7, -30]} intensity={0.15} distance={15} color="#fff" />
            <pointLight position={[0, 7, -10]} intensity={0.15} distance={15} color="#fff" />
            {ceilingLights}

            {/* Simulated Glow - Reduced 15 -> 8 */}
            <rectAreaLight position={[-19.8, HALL_HEIGHT/2, 35]} width={70} height={16} color={conferenceState.lightColor} intensity={8} rotation={[0, -Math.PI/2, 0]} />
            <rectAreaLight position={[19.8, HALL_HEIGHT/2, 35]} width={70} height={16} color={conferenceState.lightColor} intensity={8} rotation={[0, Math.PI/2, 0]} />
            
            {/* The ONLY Light that casts dynamic shadows - Reduced 3 -> 1.5 */}
            <spotLight 
                position={[0, 15, 40]} 
            ...
            {/* CONTACT SHADOWS - Realtime but Optimized */}
            {/* Frames=Infinity removes the "ghost trail" of the player. 
                Resolution reduced to 256 for performance. */}
            <ContactShadows 
                position={[0, 0.1, 35]} 
                opacity={0.8} 
                scale={60} 
